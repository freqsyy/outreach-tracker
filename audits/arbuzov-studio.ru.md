# Отчёт по тестированию сайта arbuzov-studio.ru

**Дата:** 18.07.2026
**Проверял:** Назар (ручное тестирование)
**Платформа сайта:** Tilda / самопис / WordPress
**Контакт владельца:** info@arbuzov-studio.ru
...
|---|-----|----------|----------|-------------|
| 1 | кнопка «Отправить» | Кнопка перекрыта другим элементом (DIV.row) — клик по ней визуально не срабатывает, попадает на перекрывающий блок. | — | HIGH |
| 2 | кнопка «» | Кнопка перекрыта другим элементом (DIV.col-md-7) — клик по ней визуально не срабатывает, попадает на перекрывающий блок. | — | HIGH |
...
## 4. Итог
Найдено 2 багов. Топ-приоритет: кнопка «Отправить»; кнопка «»
...
- Всего багов: 2
- 1. **[HIGH]** кнопка «Отправить»: Кнопка перекрыта другим элементом (DIV.row) — клик по ней визуально не срабатывает, попадает на перекрывающий блок.
- 2. **[HIGH]** кнопка «»: Кнопка перекрыта другим элементом (DIV.col-md-7) — клик по ней визуально не срабатывает, попадает на перекрывающий блок.
