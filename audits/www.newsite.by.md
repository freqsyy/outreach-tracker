# Отчёт по тестированию сайта www.newsite.by

**Дата:** 11.07.2026
**Проверял:** Назар (ручное тестирование)
**Платформа сайта:** Tilda / самопис / WordPress
**Контакт владельца:** contact@newsite.by
...
|---|-----|----------|----------|-------------|
| 1 | кнопка «» | Кнопка перекрыта другим элементом (DIV.preloader__overlay js-preloader-overlay) — клик по ней визуально не срабатывает, попадает на перекрывающий блок. | — | HIGH |
| 2 | кнопка «Понятно» | Кнопка перекрыта другим элементом (DIV.preloader__overlay js-preloader-overlay) — клик по ней визуально не срабатывает, попадает на перекрывающий блок. | — | HIGH |
| 3 | мобильная вёрстка (375px) | Горизонтальный переполнение на телефоне: scrollWidth=676 > clientWidth=360 (на 316px). Контент уезжает за экран. | — | MEDIUM |
...
## 4. Итог
Найдено 5 багов. Топ-приоритет: кнопка «»; кнопка «Понятно»; мобильная вёрстка (375px)
...
- Всего багов: 5
- 1. **[HIGH]** кнопка «»: Кнопка перекрыта другим элементом (DIV.preloader__overlay js-preloader-overlay) — клик по ней визуально не срабатывает, попадает на перекрывающий блок.
- 2. **[HIGH]** кнопка «Понятно»: Кнопка перекрыта другим элементом (DIV.preloader__overlay js-preloader-overlay) — клик по ней визуально не срабатывает, попадает на перекрывающий блок.
- 3. **[MEDIUM]** мобильная вёрстка (375px): Горизонтальный переполнение на телефоне: scrollWidth=676 > clientWidth=360 (на 316px). Контент уезжает за экран.
